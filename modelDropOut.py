...
from builderDropout import build_unet
old_args_map = {
...
def Unet(backbone_name='vgg16',
    ...
    if backbone_name=='efficientnet-b2':
        #img_size_target = 260
        img_size_target = 256
        backbone = EfficientNetB2(include_top=False,input_shape=(img_size_target, img_size_target,3),weights='imagenet',  pooling=None)
    if backbone_name=='efficientnet-b3':
        #img_size_target = 300
        img_size_target = 288
        backbone = EfficientNetB3(include_top=False,input_shape=(img_size_target, img_size_target,3),weights='imagenet',  pooling=None)
    if backbone_name=='efficientnet-b4':
        #img_size_target = 380
        img_size_target = 384
        backbone = EfficientNetB4(include_top=False,input_shape=(img_size_target, img_size_target,3),weights='imagenet',  pooling=None)
    if backbone_name=='efficientnet-b5':
        #img_size_target = 456
        img_size_target = 448
        backbone = EfficientNetB5(include_top=False,input_shape=(img_size_target, img_size_target,3),weights='imagenet',  pooling=None)
